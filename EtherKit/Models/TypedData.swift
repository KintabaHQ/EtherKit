...
extension Array: Signable where Element == TypedData {
    public func signatureData(_: Network?) -> Data {
        let schemas = self.map { $0.schemaData }.reduce(Data(), { $0 + $1 }).sha3(.keccak256)
        let values = self.map { $0.typedData }.reduce(Data(), { $0 + $1 }).sha3(.keccak256)
