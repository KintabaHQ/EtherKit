...
public enum Denomination: BigUInt, EnumCollection {
  case wei = 1
  case kwei = 1000
  case mwei = 1_000_000
  ...
  public static func getBestFit(value: UInt256) -> Denomination {
    for denom in Denomination.allValues.reversed() {
      let converted = Denomination.convert(value, to: denom)
      if converted > 0.01 {
        return denom
      }
    }
