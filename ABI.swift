...
extension Address: ABIValueType {
    ...
    public func toABIData(lift: @escaping (Data) -> ABIData) -> ABIData {
        var data = Data()
        let padding = 32 - self.data.count
        data.append(Data(repeating: 0, count: padding))
...
extension FunctionSelector: ABIValueType {
    ...
    public func toABIData(lift: @escaping (Data) -> ABIData) -> ABIData {
        let parameterString = self.parameterTypes.joined(separator: ",")
        let asciiSig = "\(self.name)(\(parameterString))"
        guard let asciiBytes = asciiSig.data(using: .ascii) else {
            fatalError()
        }
        let fullHash = asciiBytes.sha3(.keccak256)
        let hash = fullHash[0 ..< 4]
        return lift(hash)
    }
}
...
extension Function: ABIValueType {
    ...
    public func toABIData(lift: @escaping (Data) -> ABIData) -> ABIData {
        var data = Data()
        data.append(self.functionSelector.toABIData(lift: lift).data)
        for parameter in parameters {
            data.append(parameter.toABIData(lift: lift).data)
        }
